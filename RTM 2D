...
Real ,Allocatable, dimension(:,:,:) ::  P, Pfonte
Real ,Allocatable, dimension(:,:) :: mod_vel,sismo,U2,U4,P1,P2,P3,Img,sismo2, Img_pond_src
Real Sx,Sz,source,delta_t,P_zero,t,snap,h,rho,t0,fc,A,f_corte,fator_cerjan,fator_c,Lcab, check
Real :: c1,c2, dummy, soma
...
read(12,*) Ntiros, Lcab, Nr !Numero de tiros, Tamanho do cabo e Numero de receptores 
read(12,*) t,Tsismo !tempo de modelagem e 
read(12,*) snap !numero de snaps
read(12,*) f_corte,A,fx,fz,prof_reg !frequencia de corte, amplitude e posiçao da fonte e profundidade do receptor
...
& ,P(Nz,Nx,Ntcor),Img(Nz,Nx),sismo(Ntsismo,Nr))!Pfonte(Nz,Nx,Ntcor),Img_pond_src(Nz,Nx))
...
print*,'Numero de amostras do sismograma, Passos de tempo array de correlacao cruzada ---->', Ntsismo, Ntcor
pause

print*,'Numero de  tiros, offsetminimo (m), tamanho de cabo (m) e numero de canais ---->', ntiros, offmin2*h, Lcab*h, Nr
pause
...
!--------------------------------------------Calculo dos paineis de pressao--------------------------------------------------
   ...
   !	   conta_snap=conta_snap+1
     ...
     !endif
      if (mod(k,Tsismo)==0) then
         nsismo=nsismo+1
         do l=1,Nr
            sismo(nsismo,l)=P3(prof_reg,fx-(l*offmin2)+(f-1)*dh)
         enddo
		endif
		  ...
		  if (mod(k,ncor)==0) then
		     do j=1,Nx
		        do i=1,Nz
                 ...
                 !Img_pond_src(i,j) = Img(i,j)/(soma + dummy)
		        enddo
             enddo
				 contador=contador-1 
		  endif
...
!-----------------------------------------------imprimindo imagem---------------------------------------------------------!
     ...
     open(11,File="imagem2.bin",status="replace",access="direct",form="unformatted",recl=(comp_byte*Nx*Nz))!//trim(adjustl(numsismo))//"
	  write(11,rec=1) ((Img(i,j),i=1,Nz),j=1,Nx)
	  close(11)
...
deallocate(P1,P2,P3,mod_vel,U2,U4,Be,Bd,P,Pfonte,Img,Img_pond_src)
